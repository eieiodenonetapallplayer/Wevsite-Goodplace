...
function notifyLine($accessToken, $username, $product, $count, $price, $what) {
    $message = "
        บริการสั่งซื้อไอดีพาส
        ---------------------
        ชื่อผู้ใช้: $username
        สินค้า: $product
        จำนวน: $count
        ราคา: $price
        สถานะ: สั่งซื้อสำเร็จ
        ---------------------
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_SESSION['id'])) {
        ...
        if ($plr["point"] >= $price) {
            ...
            $ch = curl_init($notify['dis_system']);
            ...
            $insert = dd_q("INSERT INTO service_order (cosid , prod , user , pass , idps_des , count , status , del, date)
                        ...
                        if ($insert AND $upt) {
                            notifyLine($notify['line_system'], $plr['username'], $pd["name"], $_POST["count"], $price, 'บริการไอดีพาส');
